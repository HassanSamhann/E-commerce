    ...
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
            ...
            <form onSubmit={handleSearchSubmit} className="flex-1 max-w-xl mx-auto w-full relative">
              <div className="flex items-center rounded-xl overflow-hidden border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 focus-within:ring-2 focus-within:ring-brand-500/20 focus-within:border-brand-500 transition-all">
                <input
                  type="text"
            ...
            <div className="hidden md:flex items-center justify-end gap-3.5 flex-shrink-0">
              ...
              {mounted && (
                <button
                  onClick={() => setTheme(resolvedTheme === "dark" ? "light" : "dark")}
                  className="p-2.5 rounded-xl text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white hover:bg-zinc-100 dark:hover:bg-zinc-900 transition-all focus:outline-none"
              ...
              <button className="p-2.5 rounded-xl text-zinc-500 hover:text-red-500 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-900 transition-all">
              ...
              {!isCheckoutPage && (
                ...
                >
                  ...
                  {cartCount > 0 && (
                    <span
                      className="absolute -top-1.5 -right-1.5 w-5.5 h-5.5 text-[10px] font-extrabold text-white rounded-full flex items-center justify-center border-2 border-white dark:border-zinc-950 animate-pulse shadow-sm"
                      style={{ backgroundColor: store.primaryColor || taagerTeal }}
                    >
                      {cartCount}
                    </span>
      ...
      <div className="bg-white dark:bg-zinc-950 border-b border-zinc-200/80 dark:border-zinc-800/60 py-2.5 transition-colors shadow-sm text-sm font-bold relative hidden sm:block">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between gap-6" dir={isRtl ? "rtl" : "ltr"}>
              ...
              <AnimatePresence>
                {isMegaMenuOpen && (
                  <motion.div
                    ...
                    className={cn(
                      "absolute top-full mt-2 bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-2xl z-50 flex overflow-hidden min-w-[500px] h-[340px]",
                        ...
                        return (
                          <div
                            ...
                            className={cn(
                              ...
                              isActive 
                                ? "bg-white dark:bg-zinc-950 text-zinc-900 dark:text-white border-r-4"
                      ...
                      {activeParentCat ? (
                        <div className="space-y-6">
                          <div className="border-b border-zinc-100 dark:border-zinc-850 pb-2 flex items-center justify-between">
                            <h3 className="text-xs font-black text-zinc-405 uppercase tracking-widest">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                              {activeParentCat.children.map((sub: any) => (
                                <Link
                                  ...
                                  onClick={() => setIsMegaMenuOpen(false)}
                                  className="flex items-center justify-between p-3 rounded-xl border border-zinc-200 dark:border-zinc-800 hover:border-zinc-300 dark:hover:border-zinc-700 bg-zinc-100/30 dark:bg-zinc-900/30 text-xs font-bold text-zinc-700 dark:text-zinc-300 hover:text-zinc-900 dark:hover:text-white transition-all"
                                >
                                  <span>{sub.name}</span>
          ...
          <nav className="flex items-center gap-8 text-zinc-600 dark:text-zinc-400">
            <Link
              href={`/store/${params.slug}`}
              className={cn(
                "hover:text-zinc-900 dark:hover:text-white transition-colors py-1.5 border-b-2 border-transparent",
                !pathname.includes("/products/") && !isCheckoutPage && "text-zinc-900 dark:text-white"
              )}
              style={{
            ...
            <button
              ...
              className="hover:text-zinc-900 dark:hover:text-white transition-colors py-1.5 border-b-2 border-transparent flex items-center gap-1.5"
            >
        ...
        {isCartOpen && (
          <>
            ...
            <motion.div
              ...
              className={cn(
                "fixed top-0 bottom-0 z-50 w-full max-w-md bg-white dark:bg-zinc-950 shadow-2xl flex flex-col border-l border-zinc-200/80 dark:border-zinc-800/60",
            ...
            >
              ...
              <div className="flex items-center justify-between p-5 border-b border-zinc-100 dark:border-zinc-800" dir={isRtl ? "rtl" : "ltr"}>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs bg-zinc-100 dark:bg-zinc-800 text-zinc-600 px-2 py-0.5 rounded-full font-bold">
                    {cartCount} {language === "ar" ? "منتجات" : cartCount === 1 ? "item" : "items"}
                  </span>
                </div>
                <button
                  onClick={() => setIsCartOpen(false)}
                  className="p-1 rounded-lg hover:bg-zinc-50 dark:hover:bg-zinc-800 text-zinc-400 hover:text-zinc-600 dark:hover:text-white transition-colors"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-4" dir={isRtl ? "rtl" : "ltr"}>
                {cartItems.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-center text-zinc-400 dark:text-zinc-500 py-12">
                    ...
                    <h3 className="text-base font-semibold text-zinc-700 dark:text-zinc-300">{t("cartIsEmpty")}</h3>
                  ...
                  cartItems.map((item) => (
                    <div
                      key={item.id}
                      className="flex items-center gap-4 py-3 border-b border-zinc-100 dark:border-zinc-900 last:border-0"
                    >
                      {item.image ? (
                        <img
                          src={item.image}
                          alt={item.name}
                          className="w-16 h-16 rounded-lg object-cover bg-zinc-50 border border-zinc-100 dark:border-zinc-800"
                        />
                      ) : (
                        <div className="w-16 h-16 rounded-lg bg-zinc-100 dark:bg-zinc-900 flex items-center justify-center text-zinc-400">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-2">
                          <button
                            ...
                            className="p-1 rounded bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-800 dark:hover:bg-zinc-700 text-zinc-500 hover:text-zinc-700 transition-colors"
                          >
                            <Minus className="w-3.5 h-3.5" />
                          </button>
                          <span className="text-xs font-semibold w-6 text-center text-zinc-800 dark:text-white">
                            {item.quantity}
                          </span>
                          <button
                            onClick={() => updateQuantity(item.id, item.quantity + 1)}
                      ...
                      <button
                        ...
                        className="p-1.5 text-zinc-400 hover:text-red-500 dark:hover:text-red-400 rounded-lg hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors"
                      >
              ...
              {cartItems.length > 0 && (
                <div className="p-5 border-t border-zinc-100 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/50" dir={isRtl ? "rtl" : "ltr"}>
                  <div className="flex items-center justify-between text-base font-black text-zinc-800 dark:text-white mb-4">
                  ...
                  <button
                    ...
                    className="w-full py-3.5 px-4 rounded-xl text-white font-bold text-center hover:opacity-95 shadow-lg shadow-zinc-100 dark:shadow-none transition-all duration-200"
                    style={{ backgroundColor: store.primaryColor || taagerTeal }}
                  >
                    {t("proceedToCheckout")}
                  </button>
...
function StorefrontLayoutContent({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-zinc-50 dark:bg-black text-zinc-900 dark:text-zinc-200 transition-colors pb-16 sm:pb-0">
      ...
      <footer className="bg-white dark:bg-zinc-950 border-t border-zinc-200/80 dark:border-zinc-800/60 py-16 transition-colors" dir={isRtl ? "rtl" : "ltr"}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-12">
            ...
            <div className="md:col-span-2 space-y-4 text-center md:text-right">
              <div className="flex items-center justify-center md:justify-start gap-2.5">
                ...
                <span className="text-xl font-black text-zinc-900 dark:text-white tracking-tight">{data.name}</span>
              </div>
            ...
            <div className="text-center md:text-right">
              <h4 className="text-xs font-black text-zinc-400 uppercase tracking-widest mb-4">
                {isRtl ? "روابط سريعة" : "Quick Links"}
              </h4>
              <ul className="space-y-2.5 text-sm font-semibold text-zinc-600 dark:text-zinc-400">
                <li>
                  <Link href={`/store/${slug}`} className="hover:text-zinc-900 dark:hover:text-white transition-colors">
                    {isRtl ? "كتالوج المنتجات" : "Products Catalog"}
                  </Link>
                </li>
                <li>
                  <Link href={`/store/${slug}/checkout`} className="hover:text-zinc-900 dark:hover:text-white transition-colors">
            ...
            <div className="text-center md:text-right space-y-4">
              <h4 className="text-xs font-black text-zinc-400 uppercase tracking-widest mb-2">
              ...
              <ul className="space-y-3.5 text-sm text-zinc-600 dark:text-zinc-400">
                ...
                <li className="pt-2 border-t border-zinc-100 dark:border-zinc-800">
          ...
          <div className="pt-8 border-t border-zinc-100 dark:border-zinc-800 text-center text-xs text-zinc-400 font-medium">
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 bg-white/95 dark:bg-zinc-950/95 backdrop-blur-lg border-t border-zinc-200 dark:border-zinc-800 px-6 py-2.5 flex items-center justify-between sm:hidden shadow-[0_-8px_30px_rgb(0,0,0,0.06)] transition-all">
        ...
        <Link 
          href={`/store/${slug}`}
          className="flex flex-col items-center gap-1 text-zinc-500 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-white transition-all duration-200"
        >
        ...
        <button 
          onClick={() => setIsCartOpen(true)}
          className="relative flex flex-col items-center gap-1 text-zinc-500 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-white transition-all duration-200 focus:outline-none"
        >
        ...
        {mounted && (
          <button 
            onClick={() => setTheme(resolvedTheme === "dark" ? "light" : "dark")}
            className="flex flex-col items-center gap-1 text-zinc-500 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-white transition-all duration-200 focus:outline-none"
          >
            {resolvedTheme === "dark" ? (
              <Sun className="w-5 h-5 text-amber-500 transition-transform duration-500 hover:rotate-45" />
            ) : (
        ...
        {data.phone && (
          <a 
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="flex flex-col items-center gap-1 text-zinc-500 hover:text-emerald-500 dark:text-zinc-400 dark:hover:text-emerald-400 transition-all duration-200"
          >
